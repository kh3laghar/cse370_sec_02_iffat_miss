...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "UpdateForm")) {
  $updateSQL = sprintf("UPDATE vehicle_details SET vehicle_id=%s WHERE vehicle_reg_no=%s",
                       GetSQLValueString($_POST['text1'], "int"),
                       GetSQLValueString($_POST['text2'], "int"));
...
$maxRows_VehicleDetailsUpdate = 5;
$pageNum_VehicleDetailsUpdate = 0;
if (isset($_GET['pageNum_VehicleDetailsUpdate'])) {
  $pageNum_VehicleDetailsUpdate = $_GET['pageNum_VehicleDetailsUpdate'];
}
$startRow_VehicleDetailsUpdate = $pageNum_VehicleDetailsUpdate * $maxRows_VehicleDetailsUpdate;

mysql_select_db($database_user_info, $user_info);
$query_VehicleDetailsUpdate = "SELECT * FROM vehicle_details";
$query_limit_VehicleDetailsUpdate = sprintf("%s LIMIT %d, %d", $query_VehicleDetailsUpdate, $startRow_VehicleDetailsUpdate, $maxRows_VehicleDetailsUpdate);
$VehicleDetailsUpdate = mysql_query($query_limit_VehicleDetailsUpdate, $user_info) or die(mysql_error());
$row_VehicleDetailsUpdate = mysql_fetch_assoc($VehicleDetailsUpdate);

if (isset($_GET['totalRows_VehicleDetailsUpdate'])) {
  $totalRows_VehicleDetailsUpdate = $_GET['totalRows_VehicleDetailsUpdate'];
} else {
  $all_VehicleDetailsUpdate = mysql_query($query_VehicleDetailsUpdate);
  $totalRows_VehicleDetailsUpdate = mysql_num_rows($all_VehicleDetailsUpdate);
}
$totalPages_VehicleDetailsUpdate = ceil($totalRows_VehicleDetailsUpdate/$maxRows_VehicleDetailsUpdate)-1;
?>
<?php
if("" == ""){
...
<style type="text/css">
#Content #ContentRight #UpdateForm table tr td table tr td table tr th {
	text-align: center;
}
</style>
...
<div id="ContentRight">
  <form action="<?php echo $editFormAction; ?>" method="POST" name="UpdateForm" id="UpdateForm">
    <table width="600" border="0">
      <tr>
        <td><p>Account Info : <?php echo $row_user['FName']; ?>   User Name : <?php echo $row_user['user_id']; ?>
        </p>
          <p>&nbsp;</p>
          <p>&nbsp;</p>
          <table width="400" border="0" align="center">
            ...
            <tr>
              <td><table width="522" border="0">
                <tr>
                  <th width="220" scope="col">Vehicle </th>
                  <th width="188" scope="col">RegNo</th>
                  <th width="78" scope="col">Update</th>
                </tr>
                <?php do { ?>
                  <tr>
                    ...
                    <td><span class="styleTxtField" id="sprytextfield2">
                      <input name="text2" type="text" class="styleTxtField" id="text2" value="<?php echo $row_VehicleDetailsUpdate['vehicle_reg_no']; ?>">
                      <span class="textfieldRequiredMsg">A value is required.</span></span></td>
                    <td><input name="UpadateDate" type="submit" id="UpadateDate" value="Update"></td>
                  </tr>
                  <?php } while ($row_VehicleDetailsUpdate = mysql_fetch_assoc($VehicleDetailsUpdate)); ?>
                <tr>
                  <td>&nbsp;</td>
                  <td>&nbsp;</td>
                  <td>&nbsp;</td>
            ...
            <tr>
              <td><input name="UserIDHidded" type="hidden" id="UserIDHidded" form="UpdateForm" value="<?php echo $row_VehicleDetailsUpdate['vehicle_id']; ?>"></td>
            </tr>
    </table></td>
      </tr>
      <tr>
